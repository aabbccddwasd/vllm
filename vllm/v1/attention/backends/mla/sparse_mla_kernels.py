...
@triton.jit
def _accumulate_dense_prefix_attention_chunk_multihead_kernel(
    q_ptr,
    kv_ptr,
    lens_ptr,
    max_score_ptr,
    ...
    """Blocked matmul accumulate for C128A/HCA prefill.

    Loads KV tiles (BLOCK_KV rows at once) from contiguous workspace and
    uses ``tl.dot`` (tensor core MMA) for QK^T, instead of per-candidate
    scalar dot products.  The workspace is per-request; ``query_start_loc``
    provides per-request cumulative token offsets (rebased to chunk-local)
    so the kernel computes the owning request row inline — no extra buffer.

    Q and KV stay in bf16 (native load type) — no fp32 upcast.
    tl.dot with bf16 inputs uses tensor core at full throughput and
    halves register / shared memory pressure vs fp32.
    """
    token_idx = tl.program_id(0)
    head_block_idx = tl.program_id(1)
    head_offsets = head_block_idx * HEAD_BLOCK + tl.arange(0, HEAD_BLOCK)
    ...
    valid_len = tl.load(lens_ptr + token_idx)
    local_eff = tl.minimum(num_candidates, tl.maximum(valid_len, 0))

    base = tl.load(query_start_loc_ptr)
    end0 = tl.load(query_start_loc_ptr + 1 * stride_qsl) - base
    end1 = tl.load(query_start_loc_ptr + 2 * stride_qsl) - base
    end2 = tl.load(query_start_loc_ptr + 3 * stride_qsl) - base
    end3 = tl.load(query_start_loc_ptr + 4 * stride_qsl) - base
    req_idx = token_idx * 0  # zero int32
    req_idx = tl.where(token_idx >= end0, 1, req_idx)
    req_idx = tl.where(token_idx >= end1, 2, req_idx)
    req_idx = tl.where(token_idx >= end2, 3, req_idx)
    token_base = kv_ptr + req_idx.to(tl.int64) * stride_kv_t
    kv_c_offsets = tl.arange(0, BLOCK_KV)

    for kv_start in range(0, local_eff, BLOCK_KV):
        kv_remaining = local_eff - kv_start
        kv_valid = kv_c_offsets < kv_remaining
        load_mask = kv_valid[:, None] & dim_mask[None, :]

        kv_tile = tl.load(
            token_base
            + (kv_start + kv_c_offsets)[:, None] * stride_kv_c
            + dim_offsets[None, :] * stride_kv_d,
            mask=load_mask,
            other=0.0,
        )

        scores = tl.dot(q, tl.trans(kv_tile), out_dtype=tl.float32) * scale
        scores = tl.where(kv_valid[None, :], scores, -float("inf"))

        tile_max = tl.max(scores, axis=1)
        next_max = tl.maximum(running_max, tile_max)
        prev_scale = tl.exp(running_max - next_max)
        tile_weights = tl.exp(scores - next_max[:, None])

        running_acc *= prev_scale[:, None]
        running_denom *= prev_scale

        # Accumulate value-weighted sum: softmax_scores @ KV_tile
        # Invalid rows contribute zero (tile_weights = exp(-inf) = 0).
        running_acc += tl.dot(tile_weights.to(tl.bfloat16), kv_tile, out_dtype=tl.float32)
        running_denom += tl.sum(tile_weights, axis=1)
...
def accumulate_dense_prefix_sparse_mla_attention_chunk_multihead(
    q: torch.Tensor,
    kv: torch.Tensor,
    lens: torch.Tensor,
    scale: float,
    ...
    """Accumulate dense prefix attention for C128A/HCA prefill.

    Args:
        q: ``[num_tokens, num_heads, head_dim]`` bf16.
        kv: ``[num_requests, M, head_dim]`` bf16 workspace.
        lens: ``[num_tokens]`` int32, each token's valid entry count.
        scale: float, attention scale (1/sqrt(head_dim)).
        max_score, denom: ``[num_tokens, num_heads]`` fp32 state.
        acc: ``[num_tokens, num_heads, head_dim]`` fp32 state.
        num_candidates: int, maximum entries per token (padded dimension).
        head_block_size: HEAD_BLOCK tile size (1, 2, 4, 8, or 16).
        query_start_loc: optional ``[PREFILL_CHUNK_SIZE+1]`` int32 (GPU),
            per-request cumulative token offsets rebased to chunk-local.
            When None, kv is per-token (backward-compat for flat testing).
    """
    if q.dim() == 4:
        assert q.shape[1] == 1
        q = q[:, 0]
    ...
    assert q.dim() == 3, f"Expected q shape [T, H, D], got {q.shape}"
    assert kv.dim() == 3, f"Expected kv shape [R, M, D], got {kv.shape}"
    assert kv.shape[-1] == q.shape[-1]
    assert lens.shape[0] == q.shape[0]
    assert max_score.shape[0] == q.shape[0]
    assert max_score.shape[1] <= q.shape[1]
    assert denom.shape == max_score.shape
    ...
    assert head_block_size in (1, 2, 4, 8, 16), (
        f"head_block_size must be 1, 2, 4, 8, or 16, got {head_block_size}"
    )
    assert max_score.dtype == torch.float32
    assert denom.dtype == torch.float32
    assert acc.dtype == torch.float32
    ...
    if query_start_loc is None:
        assert kv.shape[0] == q.shape[0]
        _qsl = torch.zeros(5, device=q.device, dtype=torch.int32)
        _qsl[1] = q.shape[0]
    else:
        assert query_start_loc.shape[0] >= 2  # at least base + 1 request
        _qsl = query_start_loc

    num_tokens, _, head_dim = q.shape
    num_heads = max_score.shape[1]
    block_d = min(1024, triton.next_power_of_2(head_dim))
    block_kv = triton.next_power_of_2(min(num_candidates, 128))
    block_kv = max(16, min(block_kv, 64))

    # Adaptive HEAD_BLOCK: bf16 Q/KV halves register pressure.
    # Use HEAD_BLOCK=16 (all heads in one block) when KV tile is small enough
    # to keep shared memory within the 101 KB SM120 budget.
    # With num_stages=3 (worst autotune config): 16KB(Q) + 48KB(KV×3) + 32KB(acc) = 96KB ✅
    if block_kv == 16 and num_heads >= 16:
        head_block_size = 16

    grid = (num_tokens, triton.cdiv(num_heads, head_block_size))
    monitor = get_perf_monitor()
    with monitor.timing("attn.c128a.kernel_launch"):
        _accumulate_dense_prefix_attention_chunk_multihead_kernel[grid](
            q,
            kv,
            lens,
            max_score,
